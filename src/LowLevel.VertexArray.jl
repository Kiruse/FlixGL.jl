...
function Base.bind(va::VertexArray, buff::PrimitiveArrayBuffer{T}, location::Integer, size::Integer; offset::Integer = 0x0, stride::Integer = 0x0, normalized::Bool = false, force_integer::Bool = false) where T
    @assert size ∈ 1:4
    @assert location >= 0
    ...
    if force_integer
        ...
        ModernGL.glVertexAttribIPointer(location, size, gltype(T), stride, Ptr{Cvoid}(convert(UInt64, offset)))
    else
        ModernGL.glVertexAttribPointer(location, size, gltype(T), normalized, stride, Ptr{Cvoid}(convert(UInt64, offset)))
    end
    checkglerror()
end
